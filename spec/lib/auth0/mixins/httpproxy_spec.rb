...
describe Auth0::Mixins::HTTPProxy do
  ...
  %i(get delete).each do |http_method|
    context ".#{http_method}" do
      ...
      it "should call send http #{http_method} method to path defined through HTTP" do
        expect(RestClient::Request).to receive(:execute).with( { method: http_method, url: '/test', timeout: nil, headers: { params: {} }, payload: nil } )
      ...
      it 'should not raise exception if data returned not in json format (should be fixed in v2)' do
        allow(RestClient::Request).to receive(:execute).with( { method: http_method, url: '/test', timeout: nil, headers: { params: {} }, payload: nil } )
      ...
      it 'should escape path with URI.escape' do
        expect(RestClient::Request).to receive(:execute).with( { method: http_method, url: '/te%20st', timeout: nil, headers: { params: {} }, payload: nil } )
          .and_return(StubResponse.new({}, true, 200))
  ...
  %i(post put patch).each do |http_method|
    context ".#{http_method}" do
      it { expect(@instance).to respond_to(http_method.to_sym) }
      it "should call send http #{http_method} method to path defined through HTTP" do
        expect(RestClient::Request).to receive(:execute).with( { method: http_method, url: '/test', timeout: nil, headers: nil, payload: "{}" } )
          .and_return(StubResponse.new({}, true, 200))
        expect { @instance.send(http_method, '/test') }.not_to raise_error
      end

      it 'should not raise exception if data returned not in json format (should be fixed in v2)' do
        allow(RestClient::Request).to receive(:execute).with( { method: http_method, url: '/test', timeout: nil, headers: nil, payload: "{}" } )
          .and_return(StubResponse.new('Some random text here', true, 200))
        expect { @instance.send(http_method, '/test') }.not_to raise_error
        expect(@instance.send(http_method, '/test')).to eql('Some random text here')
      end
      ...
      it 'should escape path with URI.escape' do
        expect(RestClient::Request).to receive(:execute).with( { method: http_method, url: '/te%20st', timeout: nil, headers: nil, payload: "{}" } )
          .and_return(StubResponse.new({}, true, 200))
        expect { @instance.send(http_method, '/te st') }.not_to raise_error
      end
