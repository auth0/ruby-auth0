      ...
      class Client
        ...
        def get(request_options: {}, **_params)
          ...
          if code.between?(200, 299)
            (response.body.to_s.empty? ? nil : Auth0::Types::GetRiskAssessmentsSettingsResponseContent.load(response.body))
          else
        ...
        def update(request_options: {}, **params)
          ...
          code = response.code.to_i
          if code.between?(200, 299)
            (response.body.to_s.empty? ? nil : Auth0::Types::UpdateRiskAssessmentsSettingsResponseContent.load(response.body))
          else
            error_class = Auth0::Errors::ResponseError.subclass_for_code(code)
            raise error_class.new(response.body, code: code)
          end
